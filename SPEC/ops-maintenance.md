...
- Rails / Next.js の未捕捉例外は Sentry に集約する。`POST /client_errors` は Sentry 未設定時のみの補助経路として扱う
...
- 失敗したパッチはロールバックする
- 例外はまず Sentry でイベント単位に確認し、Sentry 未設定時のみフロントエンド/バックエンド双方の従来ログへ送る
- Sentry の未解決イベントを確認してから週次パッチの対象範囲を決める
