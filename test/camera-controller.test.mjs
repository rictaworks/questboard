...
test('fitToContent limits zoom to options.initialFitMaxZoom for small boards/objects', () => {
  ...
  const options = {
    ...DEFAULT_CAMERA_CONTROLLER_OPTIONS,
    initialFitMaxZoom: 0.5
  };
  const resultCustom = fitToContent(smallBounds, largeViewport, options);
  assert.ok(resultCustom.zoom <= 0.5, `Zoom should be at most 0.5 when initialFitMaxZoom is configured to 0.5 (actual: ${resultCustom.zoom})`);
});

test('fitToContent respects minZoom even if it exceeds the initialFitMaxZoom limit', () => {
  const smallBounds = {left: 10, top: 10, right: 20, bottom: 20};
  const largeViewport = {width: 1000, height: 1000};
  const options = {
    ...DEFAULT_CAMERA_CONTROLLER_OPTIONS,
    minZoom: 2.0,
    maxZoom: 4.0,
    initialFitMaxZoom: 1.0
  };
  const result = fitToContent(smallBounds, largeViewport, options);
  assert.equal(result.zoom, 2.0, `Zoom should be normalized to minZoom (2.0) rather than being inverted by initialFitMaxZoom limit`);
});
