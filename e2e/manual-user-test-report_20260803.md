...
| [#52](./pr052.md) | コメントCRUD・バッジ | ⚠️ 手順1〜9・11 ✅／手順10・12はオーナー投稿コメントの用意待ちで未実施 |
...
| 10 | コメント可 | オーナーの投稿コメントが読めること | ⏸ 未実施（下記） |
| 11 | コメント可 | 自分の投稿に「編集」「削除」が出ること | ✅ 投稿者名 `Chart Design`・日時つきで表示され、編集／削除ボタンあり。バッジも 1 になる |
| 12 | コメント可 | 他人の投稿に「編集」「削除」が出ないこと | ⏸ 未実施（下記） |

手順10・12 が未実施なのは、コメント可ロールで参加できたボード（`cutover-check-20260803`）に
オーナー投稿のコメントが1件も残っていないため。手順6で削除済みで、以後オーナー側で再投稿していない。
オーナーコメントが用意されているボードA・Bは、Chart Design が先に閲覧者として参加してしまっており、
上記のとおり再参加ではロールが変わらない。オーナー（Takizawa）が同ボードにコメントを1件投稿すれば
そのまま実施できる。
...
- PR #52 手順10・12（コメント可ロールから見た他人のコメント）
  — オーナーがコメントを1件投稿したうえで、Chart Design へ再ログインして実施する。

RSpec の権限マトリクステスト（PR #39 / #41 / #52 相当）は CI で green。
