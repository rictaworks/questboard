...
**降格を担保するのはセッションの有効期限である（`SESSION_EXPIRE_AFTER_DAYS`・既定7日）。** 上の2経路はどちらも、利用者が能動的に通らないと働かない。ログイン処理はセッションが続く限り走らず、手動再判定の導線は利用不可画面にしかないため `member` の利用者は押さない。期限が切れれば次の来訪でログイン処理が走り、そこで判定を取り直す。**降格の遅れの上限はこの値そのものである。**

利用者ごとの最終確認日時や、定期的に判定を取り直す仕組みを別に置かない。降格の担保を2か所に置くと、どちらが効いて降格したのかが読めなくなる。この設計では、判定サービスで `deny` のオーバーライドを登録しても、対象の利用者のセッションが切れるまでは効かない。即時に締め出す必要が生じた時点で、改めて設計する。
...
stateDiagram-v2
    ...
    note right of none
        ...
        ログイン処理はセッションの有効期限が切れた後の
        再来訪でも走る。これが降格の担保であり、
        降格の遅れの上限は有効期限そのものになる。
    end note
```
